...
let Database = null;
  ...
  function _settingsJid(jid) { return jid || global._codexBotJid || null; }

  async function getSettings(jid) {
      jid = _settingsJid(jid);
      if (!jid && isCacheValid(cache.settings)) return cache.settings.data;
      const rows = await qAll(
          'SELECT key, value FROM settings',
          'SELECT key, value FROM settings'
      );
      const defaults = {
          prefix: '.', mode: 'public', botname: 'Codex-MD', startmessage: true,
          chatbotpm: false, autoview: true, autoread: false, antidelete: false, stealth: false,
          autoai: false, codexagent: false, anticall: false, autoreact: false, autotyping: false, autostatus: false
      };
      const s = { ...defaults };
      const prefix = jid ? jid + ':' : '';
      for (const r of rows) {
          if (prefix && r.key.startsWith(prefix)) {
              const actualKey = r.key.slice(prefix.length);
              try { s[actualKey] = JSON.parse(r.value); } catch { s[actualKey] = r.value; }
          } else if (!prefix && !r.key.includes(':')) {
  ...
  async function updateSetting(key, value, jid) {
      jid = _settingsJid(jid);
      const dbKey = jid ? `${jid}:${key}` : key;
