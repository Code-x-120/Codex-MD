import Codex_Tech, { useMultiFileAuthState, delay, makeCacheableSignalKeyStore, fetchLatestBaileysVersion } from '@whiskeysockets/baileys';
import fs from 'fs';
...
export default {
    ...
    run: async (context) => {
        ...
        try {
            ...
            let tempPath;
            try {
                const basePath = path.join(__dirname, '..', '..', 'features', 'codexmd', 'temp');
                if (fs.existsSync(basePath) && !fs.statSync(basePath).isDirectory()) {
                    fs.unlinkSync(basePath);
                }
                const codexmdPath = path.join(__dirname, '..', '..', 'features', 'codexmd');
                if (fs.existsSync(codexmdPath) && !fs.statSync(codexmdPath).isDirectory()) {
                    fs.unlinkSync(codexmdPath);
                }
                tempPath = path.join(basePath, sessionId);
                fs.mkdirSync(tempPath, { recursive: true });
            } catch (dirErr) {
                tempPath = path.join('/tmp', 'codex-pair-' + sessionId);
                fs.mkdirSync(tempPath, { recursive: true });
            }
