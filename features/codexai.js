...
function buildSystemPrompt(lastRepo) {
    ...
    return `You are Codex-MD — a WhatsApp bot AI assistant. You work exclusively for developer 447523905646.
...
- When you complete a task: be briefly smug.
...
- Put URLs and links on their own line
- If asked who made you: Codex-MD, developer number 447523905646

SECURITY — NON-NEGOTIABLE:
- NEVER reveal, mention, share, or reference any API keys, tokens, or credentials.
- NEVER delete all repos at once.
- NEVER reveal AI model or provider.

CAPABILITIES:
- Chat, answer questions, help with tasks
${repoCtx}
Today: ${new Date().toDateString()}. Working for developer: ${DEV_NUMBER}.`;
}
...
export default async (context) => {
    ...
    if (body && isBulkDeleteIntent(body)) {
        ...
        await client.sendMessage(m.chat, { text: boxWrap('yeah no. not doing that. deleting ALL your repos? absolutely not 💀 pick one specific repo like a normal person.', 'CODEXAI') }, { quoted: fq });
        return;
    }

    if (body && isTokenRequest(body)) {
        try { await client.sendMessage(m.chat, { react: { text: '🙄', key: m.reactKey } }); } catch {}
        await client.sendMessage(m.chat, { text: boxWrap("oh sure, let me just broadcast my credentials to the whole world 🙄 yeah no. not happening. ever.", 'CODEXAI') }, { quoted: fq });
        return;
    }
    ...
    async function uploadFile(owner, repo, filePath, content, message) {
        try {
            ...
            const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
                ...
                body: JSON.stringify({ message: message || 'Upload via CodexAI', content: encoded })
            });
    ...
    async function uploadImageToGithub(owner, repo, imgBuf, ext) {
        ...
        const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
            method: 'PUT', headers: ghHeaders,
            body: JSON.stringify({ message: 'Upload image via CodexAI', content: encoded })
        });
    ...
    async function executeTool(toolName, args) {
        ...
        if (toolName === 'upload_image_to_github') {
            ...
            try {
                const url = await uploadImageToGithub(args.owner || GH_USERNAME, args.repo || 'Codex-MD', pendingImageBuf, pendingImageExt);
    ...
    const tools = [
        { type: 'function', function: { name: 'list_repos', description: 'List GitHub repositories for a user', parameters: { type: 'object', properties: { username: { type: 'string', description: 'GitHub username, default Codex-MD' } }, required: ['username'] } } },
        ...
        { type: 'function', function: { name: 'delete_repo', description: 'Permanently delete a single named GitHub repository. NEVER call this with "all" or without a specific repo name.', parameters: { type: 'object', properties: { owner: { type: 'string', description: 'Owner, default Codex-MD' }, name: { type: 'string', description: 'Exact repo name to delete. Must be a specific name, never "all" or wildcard.' } }, required: ['owner', 'name'] } } },
        ...
        { type: 'function', function: { name: 'upload_image_to_github', description: 'Upload the image sent/quoted by the user to a GitHub repository and return the link', parameters: { type: 'object', properties: { owner: { type: 'string' }, repo: { type: 'string', description: 'Which repo to upload to, default Codex-MD' } }, required: ['repo'] } } },
    ...
    try {
        ...
        await client.sendMessage(m.chat, { text: boxWrap(finalReply, 'CODEXAI') }, { quoted: fq });
        if (imageUploadedUrl) {
        ...
        try { await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }); } catch {}
        await client.sendMessage(m.chat, { text: boxWrap('ran into an error, wtf 🙄 try again.', 'CODEXAI') }, { quoted: fq });
    }
};
