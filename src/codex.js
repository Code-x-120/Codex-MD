...
import codexaiFeature from '../features/codexai.js';
...
let _codexPict = Buffer.alloc(0);
try {
    _codexPict = fs.readFileSync(path.resolve(__dirname, '../codex.jpg'));
} catch (e) {
    _codexPict = Buffer.alloc(0);
}
...
const _groupMetaCache = global._codexGroupMetaCache;
...
export default async (client, m, chatUpdate, store) => {
    ...
    try {
        ...
        var msgCodex = m.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;
        var budy = typeof m.text == "string" ? m.text : "";

        const timestamp = speed();
        const codexspeed = speed() - timestamp;
        const pict = _codexPict;
        ...
        const isSettingsOwner = !!(settings?.ownerNumber && normalizeNumber(String(settings.ownerNumber)) === senderNormalized);
        const Owner = itsMe || isDev || isSudo || isEnvOwner || isSettingsOwner;
        let text = args.join(" ");
        ...
        if (isStealthOn) {
            if (commandName === 'stealth' && cmd) {
                const stCtx = {
                    ...
                    msgCodex, botNumber, itsMe, packname: settings.packname, generateProfilePicture, groupMetadata: m.metadata || {},
                    codexspeed, mycode, fetchJson, exec, getRandom, UploadFileUgu, TelegraPh, prefix: usedPrefix, cmd,
        ...
        if (cmd) {
            try {
                ...
                const cmdCtx = {
                    ...
                    itsMe, isDev, isSudo, isEnvOwner, Owner, settings, Tag, msgCodex, budy, sock: client, store,
                    ...
                    packname: settings.packname, generateProfilePicture, codexspeed, mycode, fetchJson,
        ...
        // codexai: developer-only GitHub AI agent
        if (settings?.codexagent === true || settings?.codexagent === 'true') {
            codexaiFeature({ client, m, body, isDev, settings, fq: fakeQuoted, prefix: usedPrefix }).catch(e => console.log('❌ [CODEXAI]:', e.message));
        }

    } catch (e) {
        console.log('codex.js main error:', e.message);
    }
};